...
// console.log("form action: "+action);
    ...
    // if(formName == 'user_details') {
    //     data.append('profile_img', $("#edit-profile-img")[0].files[0]);
    // }
    ...
    axios({
        ...
        enabledFields.prop("disabled", false);
        if (error.response && (!onErrors || onErrors(form, fields, error.response)))
...
var ajaxForm = function (name, onResult, onSubmit, onErrors) {
	...
	// console.log("form ID"+ $(`form[name="${name}"]`).attr("id"));
